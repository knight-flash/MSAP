...
from TEST3 import SEModule,AdaptiveReweight,CE,PAMAP2_CE,ReparamLargeKernelConv,ELK,ELK_CNN,ChannelAttention,SpatialAttention,resnet
import sys
arg = sys.argv
name = str(arg[1])
modelfile = str(arg[2])
...
X = torch.from_numpy(np.load(datapath+'x_test.npy')[0:50]).float()
y = torch.from_numpy(np.load(datapath+'y_test.npy')[0:50]).long()
